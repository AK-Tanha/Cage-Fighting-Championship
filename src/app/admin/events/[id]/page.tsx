...
const EventDetailsPage = () => {
  ...
  const renderFighterHoverCard = (fighter: Fighter) => {
    const cardW = 340;
    const cardH = 220;
    const offset = 16;
    const x = hoverPos.x + offset + cardW > window.innerWidth
      ? hoverPos.x - cardW - offset
      : hoverPos.x + offset;
    const y = hoverPos.y + offset + cardH > window.innerHeight
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 p-4 flex flex-col justify-between min-w-0">
          <div>
            <p className="text-[#FE0002] text-[9px] font-bold uppercase tracking-widest italic mb-1">{fighter.weight_class}</p>
            <h4 className="font-display font-black uppercase text-lg leading-tight tracking-tighter mb-3 truncate">{fighter.name}</h4>
            ...
            <div className="grid grid-cols-2 gap-1.5 mb-3">
              ...
              <div className="bg-gray-50 px-2 py-1.5 border border-black/5">
                ...
                <span className="text-xs font-black italic truncate">{fighter.nationality || "N/A"}</span>
              </div>
            </div>
  ...
  return (
    <div className="space-y-6 relative">
      {/* Fighter Hover Card */}
      {fighterHoverCard && renderFighterHoverCard(fighterHoverCard)}
      ...
      <div className="bg-white border border-black/5 rounded-sm overflow-hidden shadow-sm">
        ...
        <div className="p-8 md:p-12">
          <div className="mb-8">
            ...
            <p className="text-gray-600 mb-6">Explore the full match listings and details for {event.name}.</p>
              ...
              {event && event.fights && event.fights.length > 0 ? (
                <ul className="space-y-2">
                  {event.fights.map((fight: any, index: number) => {
                    ...
                    return (
                      <li key={index} className="px-4 py-3 bg-gray-50 border border-black/5 rounded-sm flex flex-col md:flex-row items-start md:items-center justify-between text-sm gap-4">
                        <div className="flex items-center flex-wrap gap-2 md:gap-3 w-full md:w-auto">
                          <span className="font-bold text-[#FE0002] uppercase mr-1 md:mr-2.5 text-xs whitespace-nowrap">Bout {String(index + 1).padStart(2, '0')}</span>
                          ...
                          >
                            <div className="relative w-10 h-10 overflow-hidden shrink-0 bg-gray-100">
                              <Image src={f1?.image_url || f1?.image || '/og-fighter-default.jpg'} alt={f1?.name || 'Fighter'} fill className="object-cover" />
                            </div>
                            <span className="font-semibold text-xs whitespace-nowrap pr-2">{f1?.name || fight.fighter1Name || "Unknown Fighter"}</span>
                          </div>

                          <span className="mx-1 text-gray-400 font-display italic text-[10px]">VS</span>
                          ...
                          >
                            <div className="relative w-10 h-10 overflow-hidden shrink-0 bg-gray-100">
                              <Image src={f2?.image_url || f2?.image || '/og-fighter-default.jpg'} alt={f2?.name || 'Fighter'} fill className="object-cover" />
                            </div>
                            <span className="font-semibold text-xs whitespace-nowrap pr-2">{f2?.name || fight.fighter2Name || "Unknown Fighter"}</span>
                          </div>
                        </div>
                ...
                <div className="text-center py-8 bg-gray-50 border border-black/5 rounded-sm">
                  <p className="text-gray-500 text-sm font-bold uppercase tracking-widest">No fights scheduled</p>
                </div>
              )}
            </div>
          </div>
