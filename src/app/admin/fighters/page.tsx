    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h2 className="font-display text-2xl font-black uppercase tracking-tight">Fighters Management</h2>
                <button className="bg-[#FE0002] text-white px-6 py-2 text-xs font-bold uppercase tracking-widest flex items-center gap-2 hover:bg-black transition-colors rounded-sm shadow-sm active:scale-95">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse min-w-[800px]">
                        ...
                        <tbody className="text-sm font-medium">
                            {[
                                { name: 'Alex Pereira', nickname: 'Poatan', record: '11-2-0', weight: 'Light Heavyweight', status: 'Active' },
                                { name: 'Islam Makhachev', nickname: '', record: '26-1-0', weight: 'Lightweight', status: 'Active' },
                                { name: 'Jon Jones', nickname: 'Bones', record: '27-1-0', weight: 'Heavyweight', status: 'Active' },
                                { name: 'Ilia Topuria', nickname: 'El Matador', record: '15-0-0', weight: 'Featherweight', status: 'Active' },
                            ].map((fighter, i) => (
                                <tr key={i} className="border-b border-black/5 hover:bg-gray-50 transition-colors">
                                    <td className="p-6">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <div className="font-display font-black uppercase text-base tracking-tighter">{fighter.name}</div>
                                                {fighter.nickname && <div className="text-[10px] text-[#FE0002] font-bold uppercase tracking-widest">"{fighter.nickname}"</div>}
                                            </div>
                                        </div>
                                    </td>
                                    <td className="p-6 text-center font-bold text-gray-700">{fighter.record}</td>
                                    <td className="p-6 uppercase text-gray-500 text-[10px] tracking-widest font-bold">{fighter.weight}</td>
